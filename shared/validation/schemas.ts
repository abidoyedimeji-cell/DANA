...
export const signupSchema = z.object({
  ...
  phone: z.string().trim().min(1, "Phone number is required").regex(/^\+?[1-9]\d{1,14}$/, "Invalid phone number format"),
...
export const promoCodeSchema = z.object({
  code: z
    ...
    .pipe(z.string().min(6, "Invalid promo code").max(20, "Invalid promo code").regex(/^[A-Z0-9]+$/, "Promo code must contain only uppercase letters and numbers")),
});
...
export const venueSchema = z.object({
  ...
  postcode: z.string().trim().regex(/^[A-Z]{1,2}\d{1,2}[A-Z]?\s?\d[A-Z]{2}$/i, "Invalid UK postcode"),
